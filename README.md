...
| 内容管理 | 6 | 预热/刷新内容及状态查询 |
| 统计分析 | 11 | 流量、请求数、状态码、Top 域名/URL/Referer/UA、命中率等 |
| 日志管理 | 1 | 日志下载列表 |
| 工单管理 | 9 | 工单完整生命周期管理 |
...
- **智能状态检查**：修改配置前自动检查域名状态，避免在部署中提交变更
...
1. 判断源站类型（IP 或域名）
2. 查找匹配的 SSL 证书
3. 如果没有证书 → 自动申请泛域名证书并返回 DNS 验证信息
4. 如果有证书 → 直接创建域名并开启 SSL
...
域名正在部署之前的配置变更，等待几分钟后 state 恢复为 1 再操作即可。
